...
const Details = ({ position, company, companyLink, time, adresse, work }: { position: string; company: string; companyLink: string; time: string; adresse: string; work: string; }) => {
  ...
  return <li ref={ref} className="my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col justify-between text-left lg:w-[80%]">
    ...
    >
      <h3 className="font-bold capitalize text-2xl sm:text-xl xs:text-lg">  {position} &nbsp; <a className="text-primary capitalize" href={companyLink} target="_blank" rel="noreferrer" >@{company}</a>
      </h3>
  ...
  return (
    <div className="my-40 mx-auto lg:my-30 md:my-20 xs:py-10">
      ...
      <div className="w-[75%] relative mx-auto dark:text-light lg:w-[90%] md:w-full " ref={ref}>
        ...
        <motion.div style={{ scaleY: scrollYProgress }} className="absolute origin-top bg-dark w-[3px] h-full rounded-4 dark:bg-light md:w-0.5"></motion.div>
